...
class ClientsList extends StatefulWidget {
  @override
  ClientsListState createState() => new ClientsListState();
}

class ClientsListState extends State<ClientsList> {
  final List<Contact> _clients = <Contact>[];
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text('Avenue', style: TextStyle(fontSize: 25.0)),
        actions: <Widget>[
      ...
      body: new SafeArea(
        ...
        bottom: false,
        child: new ListTile(
          title: new IconButton(icon: const Icon(Icons.add), onPressed: _pushNewClient),  
          subtitle: _buildClientList(),
        ),
      ),
    );
  }
  ...
  Widget _buildRow(Contact client) {
    final bool alreadySaved = _clients.contains(client);
    if(alreadySaved) {
      return null;
    }
    return new ListTile(
    ...
    Navigator.of(context).push(
      new MaterialPageRoute<void>(
        builder: (BuildContext context) {
          return new MyHomePage(title: "Add Client");
        },
      ),
    );
  }
